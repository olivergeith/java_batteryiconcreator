...
public class BarGraphWideV4 extends AbstractIconCreator {

	protected static String	name	= "BarGraph.Wide.V4";

	public BarGraphWideV4(final RomSettings romSettings) {
		super(romSettings);
		settings.setMoveIconWithText(false);
		settings.setFontXOffset(-27);
		settings.setFontYOffset(-12);
		settings.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 28));
		settings.setReduceFontOn100(-2);
		settings.setIconXOffset(-27);
	...
	private void drawScala(final Graphics2D g2d, final boolean charge, final int percentage) {
		...
		final int zehnerdicke = imgHeight - einerdicke - 2 - 2;
		drawSegmente(g2d, charge, percentage, zehner, offsetOben, zehnerdicke, 10);
		drawSegmente(g2d, charge, percentage, einer, offsetOben + zehnerdicke + 2, einerdicke, 10);

		clearRoundArea(g2d, 0, -Math.round(imgHeight * 9 / 4), (int) Math.round(imgHeight * 11.6 / 4));
		if (settings.isDrawZeiger()) {
			drawZeiger(g2d, charge, percentage, zehner, offsetOben, zehnerdicke, 10);
			clearRoundArea(g2d, 0, -Math.round(imgHeight * 9 / 4), (int) Math.round(imgHeight * 11.1 / 4));
		}
