...
public class BarGraphWideV3 extends AbstractIconCreator {

	protected static String	name	= "BarGraph.Wide.V3";

	public BarGraphWideV3(final RomSettings romSettings) {
		super(romSettings);
	...
	private void drawScala(final Graphics2D g2d, final boolean charge, final int percentage) {
		...
		final int einerdicke = 8;
		final int zehnerdicke = 44;
		...
		// clearRoundArea(g2d, imgWidth / 2 - 1, -Math.round(imgHeight * 13 /
		// 4), (int) Math.round(imgHeight * 13.75 / 4));
		clearRoundArea(g2d, 0, Math.round(imgHeight * 13 / 4), (int) Math.round(imgHeight * 11.5 / 4));
		if (settings.isDrawZeiger()) {
			drawZeiger(g2d, charge, percentage, zehner, offsetOben + einerdicke + 2, zehnerdicke, 10);
			clearRoundArea(g2d, 0, Math.round(imgHeight * 13 / 4), (int) Math.round(imgHeight * 11.1 / 4));
		}
